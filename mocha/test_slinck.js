...
describe('pecent_encoding', function(){
  describe('#decode', function(){
    ...
    it('basic', function(){
      ...
      scenario1("Abc/dlk\ndd\t?d\rldl=zz+z%", "Abc%2Fdlk%0Add%09%3Fd%0Dldl%3Dzz%2Bz%25");
    });
    it('utf-8', function(){
...
describe('slinck', function(){
  ...
  function hostCheck(snk){ assert.equal("host",snk.host); }
  function noHostCheck(snk){ assert.equal(null,snk.host); }
  function sectionCheck(snk){ assert.equal("branch/sec/ti/on",snk.section.toString()); }
  function sectionDbCheck(snk){ assert.equal("branch/sec/ti/ondb",snk.section.toString()); }
  function pathCheck(snk){ assert.equal("index/a/b/c",snk.path.toString()); }
  function indexCheck(snk){ assert.equal("index",snk.path.toString()); }
  
  function boundXCheck(snk,x,v){ assert.equal(x+"="+v , snk.bound(x).toString()); }
  function boundEqCheck(snk){ boundXCheck(snk,"eq","a/b/c"); }
  function boundGteCheck(snk){ boundXCheck(snk,"gte","a1/b1/c1"); }
  function boundLtCheck(snk){ boundXCheck(snk,"lt","a2/b2/c3"); }
  ...
  var testCases = {
      ...
      {  description: "fragment as range between two keys", 
        tests: [hostCheck,sectionDbCheck,indexCheck,boundGteCheck,boundLtCheck] 
      },
      "branch/sec/ti/on//" : 
      ...
      {  description: "hostless fragment as range between two keys", 
        tests: [noHostCheck,sectionDbCheck,indexCheck,boundGteCheck,boundLtCheck] 
      },
      "/branch/sec/ti/on//" : 
      {  description: "hostless section", 
         tests: [noHostCheck,sectionCheck] },
...
describe('tokenator', function(){
  ...
  describe('#assert', function(){
    it('check assert success and failure', function(){
      $_.tokenator.assert("aa","aa");
      $_.tokenator.assert("aa",["qq","aa"]);
      try{
        $_.tokenator.assert("aa","bb");
      }catch(x){
        assert.equal(x.message,"Unexpected entry: aa  expected:bb, provided:aa");
        assert.equal(x.params.expected, "bb");
        assert.equal(x.params.provided, "aa");
      }
      var arr=["cc","bb"];
      try{
        $_.tokenator.assert("aa",arr);
      }catch(x){
        assert.equal(x.message,"Unexpected entry: aa  expected:cc,bb, provided:aa");
        assert.equal(x.params.expected, arr);
        assert.equal(x.params.provided, "aa");
      }
      try{
        $_.tokenator.assert("aa","bb","haha");
      }catch(x){
  ...
  describe('#Tokenator', function(){
    it('check Tokenator functionality', function(){
      var tt = $_.tokenator.Tokenator("a/b/c//dd/x/v/l", "/?&=");
...
describe('utils', function(){
  ...
  describe('#size()', function(){
    it('', function(){
      ...
      assert.equal($_.utils.size( { a: "a", b: "b" } ), 2);
      assert.equal($_.utils.size( { a: "a" } ), 1);
    });
  });
  ...
  describe('#stack()', function(){
    ...
    describe('#error()', function(){
      it('', function(){
        var e= $_.utils.error({message: "msg", a: "a", b: "not b"});
        assert.ok(e instanceof Error);
        assert.equal(e.message, "msg  a:a, b:not b");
        $_.utils.error({b: "b",c: "c"},e);
        assert.ok(e instanceof Error);
